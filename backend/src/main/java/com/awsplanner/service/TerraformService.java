...
public class TerraformService {
    ...
    // Spring-injected fallback credentials (from application.properties → .env → OS env)
    ...
    /**
     * Resolves an AWS credential value.
     * Checks System env first, then Spring-injected properties as fallback.
     */
    ...
    /**
     * Returns true if AWS credentials are available from any source.
     */
    ...
    /**
     * Returns a human-readable credential status string for the health endpoint.
     */
    ...
    public void runDeployment(String deploymentId) {
        ...
        try {
            // Pre-flight 1: Check if Terraform is installed
            checkTerraformInstallation();

            // Pre-flight 2: check AWS credentials before doing anything
    ...
    /**
     * Terminate an active deployment process.
     */
    ...
    public List<String> getDeploymentResources(String deploymentId) {
        ...
        try {
            Path workspaceDir = Paths.get(deployment.getWorkspaceDir());
            // Fast fail if there is no state file to avoid running terraform needlessly
            ...
            try (BufferedReader reader = new BufferedReader(new InputStreamReader(process.getInputStream()))) {
                ...
                while ((line = reader.readLine()) != null) {
                    ...
                    if (cleanName != null) {
                        resources.add(cleanName);
                    }
                }
            }
            process.waitFor(30, TimeUnit.SECONDS);

            // Deduplicate to show simpler summaries (e.g., if there are 2 security groups, just show "AWS Security Group (2)")
    ...
    private String humanizeResourceName(String terraformName) {
        ...
        // Skip null data resources or providers
    ...
    private void runTerraformCommand(Deployment deployment, Path workspaceDir, String... args)
        ...
        try {
            // Capture output in a dedicated thread to prevent buffer deadlock
            Thread outputThread = captureOutputAsync(process, deployment);

            boolean finished = process.waitFor(10, TimeUnit.MINUTES);
            outputThread.join(5000); // Give output thread time to flush
    ...
    /**
     * Verifies that the terraform binary is available in the system PATH.
     * Throws TerraformNotFoundException with detailed instructions if not found.
     */
    private void checkTerraformInstallation() {
        ...
        // Strategy 2: Attempt a "version" call as final validation
        if (!found) {
            try {
    ...
    private List<String> buildCommand(String... args) {
        // On Windows, terraform binary may need "terraform.exe"
        ...
        for (String dir : System.getenv("PATH").split(File.pathSeparator)) {
            if (new File(dir, binary).exists() || new File(dir, "terraform").exists()) {
                found = true;
                break;
            }
        }
        if (!found) {
            // Try the plain name — ProcessBuilder will search PATH anyway
    ...
    private ProcessBuilder buildProcess(List<String> command, Path workspaceDir) {
        ...
        // Inject AWS credentials: system env first, then Spring properties as fallback
        ...
        // Pass through optional vars if set
        String sessionToken = System.getenv("AWS_SESSION_TOKEN");
        if (sessionToken != null && !sessionToken.isBlank()) {
            env.put("AWS_SESSION_TOKEN", sessionToken);
        }

        // Disable Terraform checkpoint (avoids version check network call)
        env.put("CHECKPOINT_DISABLE", "1");
        // Disable color in CI if needed
    ...
    /**
     * Injects an environment variable into the process env map.
     * Priority: current process env var → Spring property fallback.
     */
    ...
    /**
     * Captures process stdout/stderr in a background thread to prevent
     * buffer-full deadlock. Returns the thread (caller must join it).
     */
    private Thread captureOutputAsync(Process process, Deployment deployment) {
        Thread t = new Thread(() -> {
            try (BufferedReader reader = new BufferedReader(
                ...
                String line;
                while ((line = reader.readLine()) != null) {
                    appendLog(deployment, line);
                }
            } catch (IOException e) {
                log.warn("Output capture interrupted: {}", e.getMessage());
            }
    ...
    private void parseAndSaveOutputs(Deployment deployment, Path workspaceDir)
        ...
        try {
            ...
            String publicIp    = extractStringOutput(outputs, "public_ip");
            String endpointUrl = extractStringOutput(outputs, "endpoint_url");
    ...
    private String buildResourceSummary(Deployment deployment) {
        return String.format(
            "Module: %s | Region: %s | Instance: %s | Provisioned via Terraform",
            deployment.getModuleType(),
            deployment.getAwsRegion(),
            deployment.getInstanceType()
        );
    }
    ...
    private String getAmiForRegion(String region) {
        // Amazon Linux 2023 AMIs by region (updated 2024)
